...
<div class="padding">
  ...
  <div class="box">
    <div class="box-header">
      <h2 style="display:inline"><b>Data Kapster</b></h2>
      <a href="{{ url('/data-kapster/tambah') }}" class="btn btn-success btn-md" style="color: white;float: right">+Tambah</a>
    </div>
    <br>
          ...
          <tbody class="<?php if($count == 0) echo 'text-center'; ?>">
            @if($count > 0)
              @foreach($data as $key => $value)
                <tr>
                  <td>{{ ($key+1) }}</td>
                  <td><img src="{{ ('/assets/img/kapster/'.$value->foto) }}" width="60px" alt="{{ ucwords($value->name) }}"></td>
                  ...
                  <td>
                    <a href="{{ route('edit-data-kapster',['id' => $value->id]) }}" class="btn btn-primary btn-sm" title="Edit" data-id="{{ $value->id }}"><i class="fa fa-edit"></i></a>
...
<script>
  ...
  $(".btn-hapus").click(function() {
    ...
    if(!confirm('Hapus Kapster ' + name)) return false;

      $('#hapus_form').attr('action', "/data-kapster/delete/" + id).submit();
